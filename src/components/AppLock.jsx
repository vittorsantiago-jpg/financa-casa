 ...
 *
 * • PIN é por usuário + dispositivo (chave inclui o ID do usuário)
 * • Dois usuários no mesmo dispositivo têm PINs independentes
 * • Biometria usa WebAuthn — funciona em PWA instalado no celular
 * • Trava automaticamente após 2 minutos em segundo plano
 */

// ── localStorage helpers (incluem userId na chave) ───────────────────────────
...
export default function AppLock({ children }) {
  ...
  useEffect(() => {
    const init = async () => {
      // Descobre o usuário logado para usar o ID nas chaves do PIN
      const supabase = createClient();
      const { data: { user } } = await supabase.auth.getUser();

      // Se não há sessão, apenas renderiza os filhos (middleware redireciona p/ /auth)
    ...
    const onVis = () => {
      if (document.hidden) {
        ...
        setStatus("locked"); setPin(""); bgTs = null;
      }
    };
    ...
    try {
      const cred = await navigator.credentials.create({
        publicKey: {
          ...
          user:      { id: crypto.getRandomValues(new Uint8Array(16)), name: "usuario", displayName: "Usuário" },
  ...
  if (status === "bio_setup") return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center",
      ...
      <div style={{ background:"#1e1b4b", borderRadius:24, padding:"36px 28px",
        ...
        }}>🔐 Ativar Digital / Face ID</button>
        <button onClick={() => setStatus("unlocked")} style={{
          width:"100%", background:"transparent", color:"rgba(255,255,255,.45)",
          border:"none", padding:"10px 0", fontSize:14, cursor:"pointer", fontFamily:"inherit",
        }}>Agora não</button>
      </div>
    </div>
  );
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
      ...
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
        ...
        <div style={{ width:56, height:56, borderRadius:"50%",
          ...
          justifyContent:"center", fontSize:28, color:"#fff" }}>💜</div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:11, width:"100%" }}>
          {keys.map((k, i) => {
            ...
            return (
              <button key={i}
                onClick={() => {
                  if (isBio) { const cid = lsGet(userId, "bio_cid"); if (cid) triggerBio(userId, cid); }
                  else tap(k);
                }}
                style={{
                  background:"rgba(255,255,255,.13)", border:"none", borderRadius:14,
                  padding:"17px 0", fontSize: typeof k === "number" ? 23 : 22,
                  fontWeight: typeof k === "number" ? 700 : 400,
                  color:"#fff", cursor:"pointer", WebkitTapHighlightColor:"transparent", fontFamily:"inherit",
                }}
        ...
        {status === "locked" && (
          <button onClick={resetPin} style={{
            ...
            fontSize:12, cursor:"pointer", marginTop:4, fontFamily:"inherit",
